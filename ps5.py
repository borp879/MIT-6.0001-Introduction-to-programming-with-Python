...
class PhraseTrigger(Trigger):
    ...
    def evaluate(self, story):
        return self.is_phrase_in(story.get_subject()) or self.is_phrase_in(story.get_link())
# Problem 3
...
def read_trigger_config(filename):
    ...
    for i in range(len(lines)):
        if lines[i] == "ADD":
            trigger_name = lines[i + 1]
            trigger_type = lines[i + 2]
            if trigger_type == "TITLE":
                trigger = TitleTrigger(lines[i + 3])
            elif trigger_type == "DESCRIPTION":
                trigger = DescriptionTrigger(lines[i + 3])
            elif trigger_type == "AFTER":
                trigger = AfterTrigger(lines[i + 3])
            elif trigger_type == "BEFORE":
                trigger = BeforeTrigger(lines[i + 3])
            elif trigger_type == "NOT":
                trigger = NotTrigger(triggers[lines[i + 3]])
            triggers[trigger_name] = trigger
    sorted_triggers = []
    for t_name in sorted(triggers):
        sorted_triggers.append(triggers[t_name])
