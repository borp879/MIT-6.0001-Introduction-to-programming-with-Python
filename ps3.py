...
def is_valid_word(word, hand, word_list):
    ...
    for letter in word:
        try:
            if new_hand[letter] < word.count(letter):
                valid = True
        except KeyError:
            is_valid = False
    if '*' in word:
        for vowel in VOWELS:
            wildcard_words.append(word.replace('*', vowel))
        for word in wildcard_words[:]:
            if word not in word_list:
                wildcard_words.remove(word)
        if len(wildcard_words) == 0:
            valid = False
    else:
        if word not in word_list:
            valid = False
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while number_of_hands > 0:
        ...
        game_score += play_hand(current_hand, word_list)
        print('-------------')
        number_of_hands -= 1
        if number_of_hands >0:
            if input('Would you like to replace the hand? ').lower() != 'yes':
                current_hand = deal_hand(HAND_SIZE)
    print('Total score over all hands:', game_score)
if __name__ == '__main__':
    word_list = load_words()
    play_game(word_list)
    display_hand({'j': 2, 'o': 1, 'l': 1, 'w': 1, 'n': 2})
